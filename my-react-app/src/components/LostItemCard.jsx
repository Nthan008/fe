...
export default function LostItemCard({
  ...
  return (
    <>
      {/* Card */}
      <div className={`bg-blue-900 text-white p-6 rounded-2xl shadow-lg w-full ${className}`}>
        {/* Labels */}
        <div className="flex flex-wrap gap-2 mb-1"> {/* Added mb-4 for vertical spacing */}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(59,130,246,0.5)]">
          <div
            ref={modalRef}
            className="bg-blue-100 text-[#04004E] p-8 rounded-2xl shadow-xl w-full max-w-md relative"
          >
            <button
              onClick={closeModal}
            ...
            <div className="flex flex-col space-y-4">
              {/* Labels */}
              <div className="flex flex-wrap gap-2 mb-"> {/* Added mb-4 for vertical spacing */}
                {labels.map((label, index) => (
                  <span
                    key={index}
                    className={`px-3 py-1 rounded-full text-sm font-semibold border ${
